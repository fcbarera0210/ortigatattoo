...
npm install
cp .env.example .env
# Editar DATABASE_URL, AUTH_SECRET, ADMIN_SEED_*, BLOB_STORE_ID_STORE_ID
# (opcional local: BLOB_READ_WRITE_TOKEN)
npm run db:push
npm run db:seed
npm run dev
```
...
| `BLOB_STORE_ID_STORE_ID` | ID del Blob store (`store_...`). En Vercel autentica con OIDC. |
| `BLOB_READ_WRITE_TOKEN` | (Opcional) Token estático `vercel_blob_rw_...` para local o fallback |
...
- [ ] Blob store + `BLOB_STORE_ID_STORE_ID` (y opcionalmente `BLOB_READ_WRITE_TOKEN` para local)
